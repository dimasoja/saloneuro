...
class Controller_Admin_Response extends Controller_AdminBase
{
    ...
    {
        $view = new View('scripts/admin/response/index');
        $response = ORM::factory('response');
        $view->contacts = $response->find_all()->as_array();
    ...
    public function action_delete() {
        $id = $_GET['id'];
        ORM::factory('response')->where('id','=',$id)->delete_all();
        //ORM::factory('settings')->deleteItem('response','$id');
         $this->action_index();
    }

    public function action_changepublished() {
        $model = ORM::factory('response')->where('id','=',$_GET['id'])->find();
    ...
    public function action_saveto() {
        $id = $_GET['id'];
        $to = $_GET['to'];
        $products = ORM::factory('response')->where('id','=',$id)->find();
        $products->to = $to; 
        $products->save();
        exit();
    }
