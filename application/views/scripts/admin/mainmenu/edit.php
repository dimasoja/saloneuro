...
<form method ="post" action="<?php echo URL::base(); ?>admin/mainmenu/edit/<?php if (isset($id)) echo $id; ?>" id="topmenu-form-submit">
    <h1>Добавить новый пункт меню:</h1><br/>
     <p>       
        <input class="submit button button-turquoise small-button" type="button" value="Сохранить пункт меню" onclick="validateAndSubmitEdit()" />       
        <input class="submit button button-turquoise small-button" type="button" value="Удалить пункт меню" onclick="deleteMenuMain(<?php echo $id; ?>)" />       
    </p>
...
?>>Для бизнеса</option>        
        ...
        <br>
        <h3>Родитель</h3>
        <input type="hidden" name="parent" id="parent" value="<?php if (isset($parent)) echo $parent; ?>"/><span class="parent label"><?php if (isset($parent_title)) echo $parent_title; ?></span>
        <a id="diary-fancy" href="#menu" class="submit button button-turquoise small-button">
               Выбрать родителя
        </a>
        ...
        <h3>URL - адрес</h3> <a id="diary-fancy" class="submit button button-turquoise small-button" href="#pages">
                Выбрать страницы
            </a><br/><br/><input type="text" name="uri" style="width: 652px !important;" id="uri" value="<?php if (isset($uri)) echo $uri; ?>"><br>
