    ...
    jQuery.ajax_upload(up_btn, {
        ...
        onComplete: function(file, responce) {
            ...
            if ('0' == responce) {
                ...
                setInterval(function() {jQuery('#photos_block_error').hide()}, 3000);
            } else {
                photos_count++;
                if (photos_count == 6) {
                    this.disable();
    ...
    jQuery('#rooms_count').keyup(function(e) {
        ...
        if (jQuery('#room_dimentions').is(':checked')) {
            ...
            for (i = 1; i <= this.value; i++) {
                ...
                html += "<input type=\"text\" name=\"price[" + i + "]\" id=\"price_" + i + "\" value=\"\" class=\"meter-field total_price\" />";
                html += "</li>";
            }
        }
        html += "</ul>";
...
function submitForm(is_checkout) {
    ...
    if (jQuery('#terms_and_conditions').is(':checked')) {
        ...
        if(jQuery.browser.safari){
            jQuery('body').animate( {scrollTop: jQuery("#terms_and_conditions_p").offset().top}, 1100 );
        }else{ 
            jQuery('body').animate( {scrollTop: jQuery("#terms_and_conditions_p").offset().top}, 1100 );
        }
    }
...
function calculatePrice() {
    ...
    if (user_name == "First Name") {user_name = "";}
    if (user_surname == "Last Name") {user_surname = "";}
    if (user_email == "Email") {user_email = "";}
    if (user_address == "Address") {user_address = "";}
    if (user_alternative_address == "Alternative Address") {user_alternative_address = "";}
    if (user_town == "Town") {user_town = "";}
    if (user_alternative_town == "Alternative Town") {user_alternative_town = "";}
    if (user_postcode == "Postcode") {user_postcode = "";}
    if (user_alternative_postcode == "Alternative Postcode") {user_alternative_postcode = "";}
    if (user_phone == "Telephone number") {user_phone = "";}
    if (user_mphone == "Mobile number") {user_mphone = "";}
    ...
    if (user_surname.length < 2 || user_surname.length > 50) {
        can_continue = false;
        jQuery('#surname_block').html('<div class="not_valid_divs" style="border: 1px solid red; margin: 5px; padding: 5px; width: 366px;">Please enter a valid last name.</div>');
        jQuery('#surname').focus();
    }
    
    if (user_name.length < 2 || user_name.length > 150) {
        can_continue = false;
        jQuery('#name_block').html('<div class="not_valid_divs" style="border: 1px solid red; margin: 5px; padding: 5px; width: 366px;">Please enter a valid first name.</div>');
        jQuery('#name').focus();
    }
...
function closeImg(id) {
    ...
    jQuery.post(baseurl + 'online-quotation/delimg', {imgpath: name}, function(data) {
        if ("1" == data) {
            photos_count--;
