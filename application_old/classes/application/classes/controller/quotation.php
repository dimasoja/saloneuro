...
class Controller_Quotation extends Controller_Base {
    ...
    public function action_test() {
        ...
        $pdf_html = new View('emails/quotation_pdf');
        $mpdf->WriteHTML($pdf_html, 0); /* формируем pdf */
        $mpdf->Output($_SERVER['DOCUMENT_ROOT'] . '/uploads/test.pdf', 'F');

        $quotation = ORM::factory('quotation')->where('id_quotation', '=', '143')->find();
        $email_body = new View('emails/quotation');
        $email_body->quotation = $quotation;


        $email = ATEmail::compose('default')
                ->to('m.leemp@gmail.com')
                ->from('hello@mysite.com')
                ->subject('Welcome to My Site')
                ->attachment($_SERVER['DOCUMENT_ROOT'] . '/images/1.jpg')
                ->body($email_body);
    ...
    public function action_step3() {
        ...
        if($quote->staining_area==',') $quote->staining_area='';
        $view->staining_area = $quote->staining_area;
        if($quote->lift_removal==',') $quote->lift_removal='';
        $view->lift_removal = $quote->lift_removal;
        if($quote->gap_filling==',') $quote->gap_filling='';
        $view->gap_filling = $quote->gap_filling;
        if($quote->gap_filling_wood==',') $quote->gap_filling_wood='';
        $view->gap_filling_wood = $quote->gap_filling_wood;
        if($quote->bitumen==',') $quote->bitumen='';
        ...
        if (isset($data_sale->sale))
            ...
            $view->sale=0;
        if($view->sale=='') $view->sale = 0; else Session::instance()->set('total_price', ((float)$quote->total_price_for_job)*(1-(float)$view->sale/100));
    ...
    public function action_step2() {
        ...
        if (!empty($_POST)) {
            ...
            if ($flag) {
                if(isset($_POST['day']))
                $day = trim(htmlspecialchars($_POST['day'])); else $day='0';
                if(isset($_POST['month']))
                $month = trim(htmlspecialchars($_POST['month'])); else $month='0';
                if(isset($_POST['year']))
                $year = trim(htmlspecialchars($_POST['year'])); else $year='0';
    ...
    public function action_index($link = false) {     
        ...
        if (isset($_SESSION['id'])) {
             if ($link != "") {$view->link=$link; $view->id_quotation=$_SESSION['id'];} else {$view->link=''; $view->id_quotation='';}
                ...
                // check the next available date $quotation->available_date
                  ...
                  while ($available_flag) { 
                      ...
                      if(empty($shedule_dates['id_date']) == true) {
						if((date("l", $tomorrow)=='Sunday') or (date("l", $tomorrow)=='Saturday')) {$tomorrow_constant += 86400;} else {
